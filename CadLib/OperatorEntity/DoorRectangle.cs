...
namespace CadLib.OperatorEntity
{
    class DoorRectangle
    {
        ...
        public static void writeTopLeftFasnSingleDoor(DxfModel dxf, DLocation DLocation, string[] DxfText, double height, double width, double outer_mid_space, double outer_in_space, double barHeight, double barWidth, string upOrDownLayer)
        {
            ...
            //画上面位置栅栏
            Fan.Draw(dxf, new Point3D(DLocation.X + width / 6, DLocation.Y + height*5/6, DLocation.Z), new Point3D(DLocation.X +width*4/5, DLocation.Y +  height*5 / 6, DLocation.Z), 0);
            //左边位置栅栏
            Fan.Draw(dxf, new Point3D(DLocation.X + width / 6, DLocation.Y + height / 5, DLocation.Z), new Point3D(DLocation.X + width / 6, DLocation.Y + height * 4 / 5, DLocation.Z), 0);
        }

        // //画单门，包括左位置栅栏，包括2个左门闩，2个右把手
        public static void writeLeftFasnSingleDoor(DxfModel dxf, DLocation DLocation, string[] DxfText, double height, double width, double outer_mid_space, double outer_in_space, double barHeight, double barWidth, string upOrDownLayer)
        {
            //画两个门闩
            writeDoorBarRectangle(dxf, DLocation, DxfText, height, width, outer_mid_space, outer_in_space, barHeight, barWidth);
            double downLeftX = DLocation.X + outer_mid_space - barWidth / 2 + (outer_in_space - outer_mid_space) / 2;
